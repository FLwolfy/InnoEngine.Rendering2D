...
[ShaderTarget(Rendering2DIds.spriteShaderTarget)]
public sealed class SpriteShaderTarget : ShaderTarget
{
    ...
    public override GraphDocument Expand(ShaderTargetContext context, CancellationToken cancellationToken)
    {
        ...
        GraphNodeRecord[] outputs = authored.nodes
            .Where(node => node.definitionId == Rendering2DIds.spriteSurfaceOutputNode)
            .ToArray();
        if (outputs.Length != 1)
        {
            throw new InvalidOperationException(
                "A Sprite Shader requires exactly one Sprite Surface Output.");
        }
        ...
        ShaderDefinition declaration = ShaderGraphDocument.ReadDefinition(
            authored,
        ...
        var builder = new Builder(
            ...
            context.serialization,
            context.references);
        GraphNodeRecord vertexCall = builder.Stage(
            ShaderStage.Vertex,
            vertex,
            GeometryInputs(5),
            [
        ...
        GraphEdgeRecord[] vertexConnections = authored.edges
            .Where(edge => edge.input.nodeId == output.id
                && edge.input.portId.value == "vertexOffset")
            .ToArray();
        if (vertexConnections.Length > 1)
        {
            throw new InvalidOperationException(
                "The Sprite vertex offset is connected more than once.");
        }
        ...
        {
            GraphSerializedValue literal = output.TryGetValue(
                ShaderGraphDocument.inputDefaultPrefix + "vertexOffset",
                out GraphSerializedValue? stored)
                ? stored!
                : ShaderGraphDocument.Encode(
                    ShaderGraphLiteral.Zero(ShaderSourceType.Atomic("float3")),
                    context.serialization,
                    context.references);
        ...
        {
            Varying("v_texcoord0", "float2", 0),
            Input("v_color0", "float4", ShaderIrInputKind.Varying, "color"),
            Varying("v_shape", "float", 1),
            Varying("v_effectStep", "float2", 2),
            Uniform("u_lightSampling"),
            Uniform("u_spriteMaterial"),
        ...
        for (int index = 0; index < 4; index++)
            surfaceInputs.Add(Texture("s_lightColor" + index, 3 + index));
        for (int index = 0; index < 4; index++)
            surfaceInputs.Add(Texture("s_lightDirection" + index, 7 + index));
        GraphNodeRecord surfaceCall = builder.Stage(
            ShaderStage.Fragment,
            surface,
            surfaceInputs.ToArray(),
            [("color0", "color0", ShaderIrOutputKind.Color, "", 0)]);
        GraphNodeRecord defaultSample = Sample(
            authored.nodes.FirstOrDefault(
                node => node.definitionId == Rendering2DIds.spriteTextureNode)?.id.value
            ?? "Sprite.DefaultSample");
        ...
        foreach (GraphNodeRecord node in authored.nodes)
        {
            ...
            if (node.id == output.id)
                continue;
            if (node.definitionId == Rendering2DIds.spriteTextureNode)
            {
                if (node.id != defaultSample.id)
            ...
            {
                throw new InvalidOperationException(
                    "Use explicit-stage authoring for additional programs; " +
                    "a Sprite surface cannot contain a second stage output.");
            }
            GraphNodeRecord copy = builder.Node(node.id.value, node.definitionId);
            foreach (KeyValuePair<string, GraphSerializedValue> value in node.values)
                copy.SetValue(value.Key, value.Value);
        ...
        {
            if (edge.input.nodeId == output.id && edge.input.portId.value == "vertexOffset")
                continue;
            GraphEndpoint from = samples.Contains(edge.output.nodeId)
                ? new(edge.output.nodeId, new("output." + edge.output.portId.value))
                : edge.output;
            ...
            if (to.nodeId == output.id)
            {
                ...
                if (!connected.Add(port))
                {
                    throw new InvalidOperationException(
                        "The Sprite surface input is connected more than once: " + port);
                }
        ...
        foreach (string port in new[] { "color", "normal", "emission" })
        {
            ...
            if (output.TryGetValue(ShaderGraphDocument.inputDefaultPrefix + port, out var literal))
                ...
                builder.Connect(defaultSample, "output." + port, surfaceCall, "input." + parameter);
        }
        ...
        {
            ShaderFunctionAsset? value = ShaderGraphDocument.Read<ShaderFunctionAsset?>(
                output,
                key,
                null,
                context.serialization,
                context.references);
            return value is { isMissing: false }
                ? value
                : throw new InvalidOperationException(
                    "The Sprite Target function is unavailable: " + key);
        }

        GraphNodeRecord VertexNode(GraphNodeId id)
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (vertexNodes.TryGetValue(id, out GraphNodeRecord? existing))
                return existing;
            GraphNodeRecord original = authored.FindNode(id)
                ?? throw new InvalidOperationException(
                    "The vertex offset references a missing node: " + id.value);
            if (original.definitionId is Rendering2DIds.spriteSurfaceOutputNode
                or Rendering2DIds.spriteTextureNode
                || original.definitionId == ShaderGraphDocument.outputDefinitionId)
            {
                throw new InvalidOperationException(
                    "A vertex offset cannot consume fragment-only Sprite sampling or a surface output. " +
                    "Use explicit vertex-stage inputs and functions.");
            }
            GraphNodeRecord copy = builder.Node("Sprite.Vertex/" + original.id.value, original.definitionId);
            vertexNodes.Add(id, copy);
            foreach (KeyValuePair<string, GraphSerializedValue> value in original.values)
                copy.SetValue(value.Key, value.Value);
            builder.Set(copy, "stage", ShaderStage.Vertex.ToString());
            if (original.definitionId == "inno.shader.stage-input")
            {
                ShaderGraphInputSettings input = ShaderGraphDocument.Read(
                    original,
                    "settings",
                    new ShaderGraphInputSettings(),
                    context.serialization,
                    context.references);
            ...
            {
                builder.Connect(
                    VertexNode(edge.output.nodeId),
                    edge.output.portId.value,
                    copy,
                    edge.input.portId.value);
            }
            return copy;
        }
        ...
        GraphNodeRecord Sample(string name)
        {
            ...
            builder.Connect(
                builder.GetInput(ShaderStage.Fragment, Varying("v_texcoord0", "float2", 0)),
                "value",
                call,
                "input.uv");
            builder.Connect(
                builder.GetInput(ShaderStage.Fragment, Varying("v_shape", "float", 1)),
                "value",
                call,
                "input.primitive");
            int slot = 0;
            ...
            {
                builder.Connect(
                    builder.GetInput(ShaderStage.Fragment, Texture(texture, slot++)),
                    "value",
                    call,
                    "input." + texture);
            }
    ...
    private static string Parameter(string port)
        => port switch
        {
            "color" => "surfaceColor",
            "normal" => "surfaceNormal",
            "emission" => "surfaceEmission",
            _ => throw new InvalidOperationException("Unknown Sprite surface input: " + port)
        };
}
...
{
    /// <inheritdoc />
    public string definitionId => Rendering2DIds.spriteSurfaceOutputNode;
    /// <inheritdoc />
...
{
    /// <inheritdoc />
    public string definitionId => Rendering2DIds.spriteTextureNode;
    /// <inheritdoc />
    public IReadOnlyList<ShaderNodePort> GetPorts(ShaderNodeDescriptionContext context)
        =>
        [
...
[ShaderGraphTemplate(Rendering2DIds.spriteShaderTemplate, "2D / Sprite Surface")]
public sealed class SpriteShaderTemplate : ShaderGraphTemplate
{
    ...
    public override GraphDocument Create(SerializationRegistry serialization, SerializationContext context)
        => Create(
            Assets.Load<ShaderFunctionAsset>(Assets.LocalPath("Shaders/SpriteVertex.ishadersource")),
            Assets.Load<ShaderFunctionAsset>(Assets.LocalPath("Shaders/SpriteSurface.ishadersource")),
            Assets.Load<ShaderFunctionAsset>(Assets.LocalPath("Shaders/SpriteSample.ishadersource")),
            serialization,
            context);
    ...
    public static GraphDocument Create(
        ...
        ShaderFunctionAsset surface,
        ShaderFunctionAsset sample,
        ...
        SerializationContext context)
    {
        ...
        ShaderGraphDocument.SetTarget(
            graph,
            Rendering2DIds.spriteShaderTarget,
            serialization,
            context);
        GraphNodeRecord output = Node(
            "surface",
            Rendering2DIds.spriteSurfaceOutputNode,
            650,
            110);
        Set(output, "vertexSource", vertex);
        Set(output, "surfaceSource", surface);
        Set(output, "sampleSource", sample);
        GraphNodeRecord texture = Node(
            "sprite-texture",
            Rendering2DIds.spriteTextureNode,
            30,
            40);
        GraphNodeRecord tint = Node("tint", "inno.shader.stage-input", 30, 320);
        Set(tint, "settings", new ShaderGraphInputSettings
        {
            id = "tint",
            type = new() { id = "float4" },
        ...
        Connect(multiply, "value", output, "color");
        return graph;
        ...
        void Set<T>(GraphNodeRecord node, string key, T value)
            => node.SetValue(
                key,
                ShaderGraphDocument.Encode(value, serialization, context));
